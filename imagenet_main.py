'''Train ISL on ImageNet with PyTorch.'''
...
parser.add_argument('--pos_score_threshold', default=0.3, type=float,
                    help='for adding neighbour')
parser.add_argument('--pos_aff_threshold', default=0.3, type=float,
...
parser.add_argument('--backbone_epoch', default=100, type=int,
...
SIZE = 227 if 'alex' in args.arch.lower() else 224

trainset = datasets.ImageFolderInstance(
    traindir,
    transforms.Compose([
...
if args.test_only or len(args.resume) > 0:
    ...
    if 'scheduler' in checkpoint.keys() and checkpoint['scheduler'] is not None:
...
logs_dir = os.path.join('logs', 'ImageNet', args.arch, str(start_datetime))
    ...
    for round_idx in range(start_round, args.round):
        for epoch_idx in range(start_epoch, args.backbone_epoch):
            ...
            if (epoch_idx + 1) % args.eval_interval == 0:
                acc = kNN(epoch_idx, net, lemniscate, trainloader, testloader, 1, args.nce_t, False)
                scheduler.step(acc)
